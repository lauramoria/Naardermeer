...
#' print(p)
# gebiedData <- EKRset[EKRset$GHPR_level == '2V21 Soortensamenstelling macrofyten',]
# df <-gebiedData

polarHistogram <-function (df, family = df$family, columnNames = NULL, binSize = 1,
                           ...
                           direction = "inwards", familyLabels = TRUE, normalised = TRUE)
{
    ...
    df <- df[!is.na(df$gebied) & !is.na(df$locatie) & 
             !(is.na(df$Waardebepalingsmethode.code)) & 
             !(is.na(df$GHPR)) & !(is.na(df$gafnaam)) & !(is.na(df$KRWwatertype.code.y))& 
  ...
  p <- p + geom_text(aes(x = x, label = item, angle = angle,
                         hjust = hjust), y = 1.02, size = 2, vjust = 0.5, data = dfItemLabels)
  ...
  p <- p + geom_text(aes(x = x, y = y, label = label), data = guideLabels,
                     angle = -alphaStart * 180/pi, hjust = 1, size = 1)
  if (familyLabels) {
  ...
  p <- p + scale_fill_manual(values=col, labels =labels)
  #p <- p + ggtitle(paste("EKR score op maatlat",df$GHPR,"voor watertype ", df$KRWwatertype.code.y, sep=" "))
  p
  #ggsave(file = paste(getwd(),"/output/polarBar",titel,format(Sys.time(),"%Y%m%d%H%M"),".png", sep= ""))
}
